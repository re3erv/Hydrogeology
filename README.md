...
### A1.1. Что делать после `check_environment.py` (типовой случай для Windows)

Если скрипт показывает, что `mf6`, `mf2005`, `mfnwt` найдены, а единственное предупреждение — версия Python `3.14`, то это означает:

- окружение **почти готово**;
- можно пробовать запуск учебных тестов и первых моделей;
- при проблемах установки/совместимости научных пакетов лучше перейти на Python `3.11` (или `3.12`/`3.13`).

Рекомендуемый порядок действий:

1. Запустить проверочные тесты из текущего интерпретатора:
   - `python -m pytest -q`
2. Если тесты зелёные — переходить к фазе **A2** (установка и проверка PM8), затем к фазе **B**.
3. Если есть ошибки, связанные именно с Python `3.14`, создать отдельное окружение на Python `3.11` и повторить:
